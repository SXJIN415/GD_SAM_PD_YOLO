...
def seg_with_sam_with_box_prompt(image, predictor, show_img=False):
...
def get_masks(image_files_path, save_label_path, single=True):
    ...
    for i in range(len(image_files_name)):
        ...
        masks, scores = seg_with_sam_with_box_prompt(image, predictor)

        seg_label = mask2txt(masks, w, h)

        number, _ = image_files_name[i].rsplit('.', 1)
        with open(os.path.join(save_label_path, number + '.txt'), "w") as f:
