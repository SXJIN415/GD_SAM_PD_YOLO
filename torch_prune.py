...
import torch
from ultralytics.nn.modules import Bottleneck, Conv, C2f, SPPF, Detect, C3k2, Segment
...
def do_pruning(modelpath, savepath, rate=0.8):
    pruning = PRUNE()

    ### 0. 加载模型
    yolo = YOLO(modelpath)  # build a new model from scratch
    pruning.get_threshold(yolo.model, rate)  # 这里的0.8为剪枝率。
    ...
    yolo.export(format="onnx", dynamic=True, simplify=True)


if __name__ == "__main__":
    modelpath = " "  # Weight files requiring pruning
    savepath = " "  # Storage address of weight file after pruning
    do_pruning(modelpath, savepath, rate=0.6)  # Pruning rate
